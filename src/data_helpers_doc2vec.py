...
def load_multiple_doc2vec(directory = 'data/Sogou/training_data/'):
    if os.path.isdir(directory):
        data_set = [str(directory + x)
                        for x in os.listdir(directory) if x[-4:] == '.txt']
    else:
    ...
    for eachEntry in data_set:
        ...
        tempList = [re.split('。|，', s) for s in tempList]
        res = []
        for s in tempList:
            currList = []
            for seg in s:
...
def load_data_doc2vec(sentences=None, labels=None, doc2vec_directory = None):
    """
    ...
    try:
        if not (len(sentences) > 0 and len(labels) > 0):
            sentences, labels = load_multiple_doc2vec()
    except:
        sentences, labels = load_multiple_doc2vec()
    testCase = 5405
    ...
    model = Doc2Vec.load(doc2vec_directory)
